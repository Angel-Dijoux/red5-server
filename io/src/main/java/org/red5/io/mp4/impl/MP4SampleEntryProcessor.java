...
public class MP4SampleEntryProcessor {
    ...
    public static void decodeStblBox(Box sbox, MP4TrackInfo trackInfo, boolean isAudio, boolean isVideo, int scale) {
    ...
    private static void decodeSampleDescriptionBox(SampleDescriptionBox stsd, MP4TrackInfo trackInfo, boolean isAudio, boolean isVideo, int scale) {
            ...
            switch (fourcc) {
                ...
                case "wave":
                    if (box instanceof WaveExtension) {
                        ...
                        wave.getBoxes().forEach(inner -> {
                            if (inner instanceof EsdsBox) {
                                EsdsBox esds = (EsdsBox) inner;
                                trackInfo.setAudioDecoderBytes(esds.getStreamInfo() != null ? esds.getStreamInfo().array() : MP4Reader.EMPTY_AAC);
    ...
    private static void handleEsdsBox(Box esdsBox, MP4TrackInfo trackInfo) {
        if (esdsBox instanceof ShortEsdsBox) {
            ...
            trackInfo.setAudioDecoderBytes(s.hasStreamInfo() ? s.getStreamInfo().array() : MP4Reader.EMPTY_AAC);
        } else if (esdsBox instanceof EsdsBox) {
            EsdsBox e = (EsdsBox) esdsBox;
            trackInfo.setAudioDecoderBytes(e.getStreamInfo() != null ? e.getStreamInfo().array() : MP4Reader.EMPTY_AAC);
        }
        trackInfo.determineAudioCodecType();
    }
            ...
            switch (fourcc) {
                ...
                case "esds":
                    if (box instanceof EsdsBox) {
                        ...
                        trackInfo.setVideoDecoderBytes(e.getStreamInfo() != null ? e.getStreamInfo().array() : MP4Reader.EMPTY_AAC);
                    }
                    break;
