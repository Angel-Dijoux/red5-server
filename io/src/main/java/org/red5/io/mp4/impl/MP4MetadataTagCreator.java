...
public class MP4MetadataTagCreator {
  ...
  public ITag createFileMeta() {
    ...
    double durationSeconds =
        (trackInfo.getTimeScale() != 0)
            ? trackInfo.getDuration() / (double) trackInfo.getTimeScale()
            : 0.0;
  ...
  private Map<String, Object> createAudioTrackInfo() {
    ...
    if (trackInfo.getAudioSamples() != null && trackInfo.getAudioSampleDuration() > 0) {
      audioMap.put(
          "length_property",
          trackInfo.getAudioSampleDuration() * trackInfo.getAudioSamples().length);
    }
    return audioMap;
  }
  ...
  private Map<String, Object> createVideoTrackInfo() {
    ...
    if (trackInfo.getVideoSamples() != null && trackInfo.getVideoSampleDuration() > 0) {
      videoMap.put(
          "length_property",
          trackInfo.getVideoSampleDuration() * trackInfo.getVideoSamples().length);
    }
    return videoMap;
  }
}
